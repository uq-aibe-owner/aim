...
function T(wVal, grid, β, f ; compute_policy = false)
    ...
    wFunc(x, y) = extrapolate(interpolate((LinRange(gridMin,gridMax,numPoints1D),LinRange(gridMin,gridMax,numPoints1D)), wVals, Gridded(Linear())), Interpolations.Line())(x,y)
    ...
    for n in 1:length(grid)
        ...
        for i in 1:numSectors
            @constraint(modTrial, gridMin <= c[i] <= y[i]-0.0001)
...
wVal = T(vStarApprox, grid, β, f; compute_policy = true)
