...
@variable(model, S[1:numSectors, 1:numSectors])
@variable(model, F[1:2*numSectors-1])
register(model, :u′, 1, u′; autodiff = true)
register(model, :S, 1, S; autodiff = true)
for i in 1:(2*numSectors-1)
    if i < numSectors
        ...
        @NLconstraint(model, F[i] == (f′(kp(s(S(S39)), y))[j] * β * u′(c(S(g(f(kp(s(S(S39)), y)))), f(kp(s(S(S39)), y))))[j] / u′(c(S(S39), y))[j]) - (S39[i] / (σ[j,j] * s(S(S39))[j])).^(1-ρ)) # -ρ / (1 - ρ) put back inside exponent
    end
end
